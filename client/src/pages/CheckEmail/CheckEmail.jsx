...
const CheckEmail = () => {
  ...
  const handleResend = async () => {
    try {
      ...
      alert('Email khôi phục đã được gửi lại!');
    } catch (err) {
  ...
  return (
    <div className="bg-background-light dark:bg-background-dark min-h-screen flex flex-col text-text-main dark:text-white relative overflow-x-hidden selection:bg-primary/30 font-display">
      ...
      <main className="flex-grow flex flex-col items-center justify-center p-6 w-full max-w-[1440px] mx-auto relative z-10">
        <div className="w-full max-w-[440px]">
          <div className="bg-white dark:bg-[#1A1D2D] rounded-xl shadow-soft dark:shadow-none border border-white/50 dark:border-white/5 p-8 sm:p-10 flex flex-col items-center text-center relative overflow-hidden group transition-shadow duration-300">
            ...
            <p className="text-text-muted dark:text-gray-400 text-sm mb-8 leading-relaxed font-sans max-w-xs mx-auto">
            ...
            <div className="w-full flex flex-col gap-4">
              <button onClick={() => window.open('https://mail.google.com', '_blank')} className="w-full bg-primary hover:bg-primary-dark text-white font-bold py-3.5 px-6 rounded-xl shadow-md hover:shadow-lg hover:shadow-primary/20 transition-all transform active:scale-[0.98] font-sans flex items-center justify-center gap-2">
            ...
            <div className="mt-8 pt-8 border-t border-gray-100 dark:border-gray-800 w-full flex justify-center">
              <Link to="/login" className="text-sm text-text-muted dark:text-gray-400 hover:text-primary dark:hover:text-primary font-medium flex items-center gap-2 transition-colors font-sans group/link">
      ...
      <footer className="w-full py-8 text-center text-xs text-text-muted/60 font-sans relative z-10">
